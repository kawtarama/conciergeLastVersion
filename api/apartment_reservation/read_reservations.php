...
//     try {
//         // Prepare the SQL query to fetch reservations from Apreservations table
//         $stmt = $conn->prepare("
//             SELECT id, apartment_id, start_date, end_date, status, phone_number, created_at, updated_at
//             FROM Apreservations
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        // Prepare the SQL query to fetch reservations from Apreservations table
        $stmt = $conn->prepare("
            SELECT id, apartment_id, start_date, end_date, status, phone_number, name, created_at, updated_at -- Added 'name' to SELECT
            FROM Apreservations
            ORDER BY start_date DESC
        ");
        $stmt->execute();
