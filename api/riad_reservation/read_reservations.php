...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        // Prepare the SQL query to fetch reservations from riadreservations table, including the 'name' column
        $stmt = $conn->prepare("
            SELECT id, riad_id, start_date, end_date, status, phone_number, name, created_at, updated_at
            FROM riadreservations
